...
const examples = {
  ...
  sort: [
  "// Insertion sort, verified: the result is sorted and is a permutation of",
  "// the input (multiset equality - no element invented, none lost).",
  "// This one is meant to be run: press Run to watch it sort.",
  ...
  "",
  "method Main() {",
  "  var a := new int[7] [31, 4, 15, 9, 26, 5, 3];",
  "  print \"before: \", a[..], \"\\n\";",
  "  InsertionSort(a);",
  "  print \"after:  \", a[..], \"\\n\";",
  "}"
...
let boogieFullText = "";
const boogieHidePlumbing = document.querySelector("#boogie-hide-plumbing");
boogieHidePlumbing.addEventListener("change", () => renderBoogieView());

// The displayed slice: with "hide type axioms" on, start at the first
// procedure/implementation — the module-level type-descriptor axioms
// (Tclass/Tag/Box/$Is, generated per program) sit above it.
function boogieDisplayText() {
  if (!boogieHidePlumbing.checked) return boogieFullText;
  const candidates = ["\nprocedure", "\nimplementation"]
    .map(marker => boogieFullText.indexOf(marker))
    .filter(index => index >= 0);
  if (candidates.length === 0) return boogieFullText;
  const start = Math.min(...candidates) + 1;
  const hidden = boogieFullText.slice(0, start).split("\n").length;
  return "// (" + hidden + " lines of generated type-descriptor axioms hidden — " +
    "uncheck 'hide type axioms' to show)\n\n" + boogieFullText.slice(start);
}

function renderBoogieView() {
  if (!boogieEditor || !boogieFullText) return;
  const shown = boogieDisplayText();
  boogieEditor.dispatch({ changes: { from: 0, to: boogieEditor.state.doc.length, insert: shown } });
  ...
  for (let line = cursorLine; line >= 1 && target < 0; line--) {
    target = shown.indexOf("/input.dfy(" + line + ",");
  }
  if (target < 0) {
    target = shown.indexOf("\nimplementation") + 1;
  }
  if (target > 0) {
